 ...
 * @since 1.0
 */
final class IncludeItem extends AbstractClassItem {

    IncludeItem(Class<?> value) {
        super(value);
    }

    private IncludeItem(Class<?> value, Set<Item<Class<?>>> slots) {
        super(value, slots);
    }

    @Override
    public Item<Class<?>> newInstance(Class<?> value, Set<Item<Class<?>>> slots) {
        return new IncludeItem(value, rebuildSlots(slots));
    }

    @Override
    public Item<Class<?>> newReverseInstance(Class<?> cls) {
        return new ExceptedItem(cls);
    }
